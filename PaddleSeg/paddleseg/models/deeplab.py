...
class DeepLabV3P(nn.Layer):
    ...
    Args:
        ...
        backbone (paddle.nn.Layer): Backbone network, cDeepLabV3urrently support Resnet50_vd/Resnet101_vd/Xception65.
...
class DeepLabV3(nn.Layer):
    ...
    def __init__(self,
        ...
        self.head2 = DeepLabV3Head(128, [0],
                                  [256], [2, 16, 32, 64],
                                  128, align_corners)

        self.head3 = DeepLabV3Head(128, [1],
                                   [512], [2, 8, 16, 32],
                                   256, align_corners)

        self.head = DeepLabV3Head(256, backbone_indices,
                                  backbone_channels, aspp_ratios,
                                  aspp_out_channels, align_corners)
        self.cls_conv = nn.Sequential(
            conv2d_bn_relu(512, 512, kernel_size=3, padding=1),
            conv2d_bn_relu(512, 256, kernel_size=3, padding=1),
            conv2d_bn_relu(256, num_classes, kernel_size=3, padding=1))
    ...
    def forward(self, x):
        ...
        #     print('feat_list=', feat_list[i].shape)
        # print('self.backbone=', self.backbone)

        logit_list2 = self.head2(feat_list)
        logit_list3 = self.head3(feat_list)

        logit_list = self.head(feat_list)


        logit_list = logit_list + logit_list2 + logit_list3

        # print('logit_list2[0].shape[2:]=', logit_list2[0].shape[2:])
        logit_list = [F.interpolate(
            logit,
            logit_list2[0].shape[2:],
            mode='bilinear',
            align_corners=self.align_corners) for logit in logit_list]

        logit_list = paddle.concat(logit_list,1)

        logit_list = [self.cls_conv(logit_list)]
...
class DeepLabV3Head(nn.Layer):
    ...
    def __init__(self, num_classes, backbone_indices, backbone_channels,
        ...
        # print('backbone_indices=', backbone_indices)
        # print('backbone_channels=', backbone_channels)
        # print('aspp_ratios=', aspp_ratios)
        # print('aspp_out_channels=', aspp_out_channels)
    ...
    def forward(self, feat_list):
        ...
        # print('feat_list=', feat_list[-1].shape)
        # print('self.backbone_indices[0]=', self.backbone_indices)
        # input('kkk')
        logit_list = []
