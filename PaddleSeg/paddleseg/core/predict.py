...
def predict(model,
    ...
    with paddle.no_grad():
        for i, im_path in enumerate(img_lists[local_rank]):
            ...
            added_image_size = (int(added_image.shape[0]/2), int(added_image.shape[1]/2))

            added_image = cv2.resize(added_image, added_image_size, interpolation=cv2.INTER_LINEAR)
            ...
            pred_size = (int(pred.shape[0]/2), int(pred.shape[1]/2))
            pred = cv2.resize(pred, pred_size, interpolation=cv2.INTER_LINEAR)
